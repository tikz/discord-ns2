...
_AWARD_RUSH_TECH = 'select playerName, r.gameTime time, ri.winningTeam as win{} from Research r inner join (select *, max(commanderTime) from PlayerRoundStats where teamNumber = {} group by roundId) prs on prs.roundId = r.roundId and prs.commanderTime > 0 and prs.teamNumber = {} inner join RoundInfo ri on ri.roundId = r.roundId where r.researchId = "{}" order by r.gameTime asc'
_AWARD_RUSH_BUILDING = 'select prs.playerName, b.gameTime, ri.roundId, ri.mapName, ri.roundDate as time, ri.winningTeam as win{} from Buildings b inner join (select *, max(commanderTime) from PlayerRoundStats where teamNumber = {} group by roundId) prs on prs.roundId = b.roundId and rs.commanderTime > 0 and prs.teamNumber = 2 inner join RoundInfo ri on ri.roundId = b.roundId where b.techId = "{}" and b.gameTime != 0 order by gameTime asc'
