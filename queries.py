...
PLAYER_CHART_RIFLE = 'SELECT roundId, 100*(hits-onosHits)/(hits+misses) val from PlayerWeaponStats where steamId = {} and weapon = "Rifle" and hits+misses != 0'
PLAYER_CHART_SHOTGUN = 'SELECT roundId, 100*(hits-onosHits)/(hits+misses) val from PlayerWeaponStats where steamId = {} and weapon = "Shotgun" and hits+misses != 0'
PLAYER_CHART_LERK = 'SELECT pcs.roundId, pws.playerDamage/pcs.classTime val from PlayerClassStats pcs inner join PlayerWeaponStats pws on pws.roundId = pcs.roundId and pws.steamId = pcs.steamId where pcs.steamId = {} and pcs.class= "Lerk" and pws.weapon = "LerkBite" and pcs.classTime != 0'
PLAYER_CHART_FADE = 'SELECT pcs.roundId, pws.playerDamage/pcs.classTime val from PlayerClassStats pcs inner join PlayerWeaponStats pws on pws.roundId = pcs.roundId and pws.steamId = pcs.steamId where pcs.steamId = {} and pcs.class= "Fade" and pws.weapon = "Swipe" and pcs.classTime != 0'
PLAYER_CHART_ONOS = 'SELECT pcs.roundId, pws.playerDamage/pcs.classTime val from PlayerClassStats pcs inner join PlayerWeaponStats pws on pws.roundId = pcs.roundId and pws.steamId = pcs.steamId where pcs.steamId = {} and pcs.class= "Onos" and pws.weapon = "Gore" and pcs.classTime != 0'